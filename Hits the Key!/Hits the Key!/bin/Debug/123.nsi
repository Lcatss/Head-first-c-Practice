...
; 安装完成页面
!define MUI_FINISHPAGE_RUN "$INSTDIR\WindowsFormsApplication1.exe"
...
Section "MainSection" SEC01
  ...
  File "WindowsFormsApplication1.exe"
  CreateDirectory "$SMPROGRAMS\My application"
  CreateShortCut "$SMPROGRAMS\My application\My application.lnk" "$INSTDIR\WindowsFormsApplication1.exe"
  CreateShortCut "$DESKTOP\My application.lnk" "$INSTDIR\WindowsFormsApplication1.exe"
SectionEnd
...
Section -Post
  ...
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\WindowsFormsApplication1.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\WindowsFormsApplication1.exe"
...
Section Uninstall
  ...
  Delete "$INSTDIR\WindowsFormsApplication1.exe"
